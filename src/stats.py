...
def stats_from_gear_view(view, *, active: bool = True) -> EquipmentStats:
    ...
    return EquipmentStats(
        might=view.stat_power,
        warding=view.stat_defense,
        fortune=view.stat_fortune,
        finesse=view.stat_insight,
        vitality=view.stat_hp,
    )
...
def get_total_equipment_stats(session: Session, player_id: int, *, player_realm_index: int | None = None) -> EquipmentStats:
    ...
    for eq in _get_player_equipment(session, player_id):
        ...
        total.might += row.might
        total.warding += row.warding
        total.fortune += row.fortune
        total.finesse += row.finesse
        total.vitality += row.vitality
    return total
...
def equipment_stats_to_modifiers(stats: EquipmentStats) -> dict[str, float]:
    ...
    return {
        "adventure_luck": stats.might * 0.008 + stats.finesse * 0.005,
        "damageBonus": stats.might * 0.004,
        "damageReduction": stats.warding * 0.010,
        "dropBonus": stats.fortune * 0.012,
    }
...
def _gear_stat_display_bits(view, *, active: bool = True) -> list[str]:
    ...
    stat_bits: list[str] = []
    might = int(getattr(view, "stat_power", 0) or 0)
    warding = int(getattr(view, "stat_defense", 0) or 0)
    fortune = int(getattr(view, "stat_fortune", 0) or 0)
    finesse = int(getattr(view, "stat_insight", 0) or 0)
    vitality = int(getattr(view, "stat_hp", 0) or 0)

    if path == "hp":
        if vitality:
            stat_bits.append(f"Vitality {vitality}")
    elif might:
        stat_bits.append(f"{_path_power_label(path)} {might}")
    if warding:
        stat_bits.append(f"Warding {warding}")
    if fortune:
        stat_bits.append(f"Fortune {fortune}")
    if finesse:
        stat_bits.append(f"Finesse {finesse}")
...
def format_stats_summary(session: Session, player_id: int, player=None, mod=None) -> str:
    ...
    stat_rows = (
        ("Vitality", combat.max_hp, "hp"),
        ("Armor", combat.armor, "armor"),
        ("Might", combat.might, "might"),
        ("Qi Power", combat.qi_power, "qi_power"),
        ("Speed", combat.speed, "speed"),
        ("Perception", combat.perception, "perception"),
        ("Resolve", combat.resolve, "resolve"),
    )
    ...
    derived = cfg["derived"]
    realm_crit = realm_stats["perception"] * derived["crit_per_perception"]
    realm_dodge = realm_stats["speed"] * derived["dodge_per_speed"]
    ...
    lines.extend([
        ...
        "",
        "**Gear totals**",
        format_stat_line("Might", total.might),
        format_stat_line("Warding", total.warding),
        format_stat_line("Vitality", total.vitality),
        format_stat_line("Fortune", total.fortune),
        format_stat_line("Finesse", total.finesse),
    ])
